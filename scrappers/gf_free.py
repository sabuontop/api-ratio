...
async def get_stats(headless: bool = False) -> Dict[str, Any]:
    try:
        ...
        token = os.getenv("GFREE_TOKEN")
        if not token:
            raise MissingCredentialsError("Missing Generation free api token")
        ...
        try:
            ...
            raise ScrappingError(f"Failed to get Generation free stats : HTTP {e.code}, Reason {e.reason}")
